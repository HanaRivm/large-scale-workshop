...
void call_method_delete(jobject chordObject, char* key, char** out_error) {
    ...
    // Call the delete method - As the method returns void, we use CallVoidMethod
...
char** call_method_get_all_keys(jobject chordObject, char** out_error)
{
    ...
    result = malloc((len + 1) * sizeof(char*)); // Allocate memory for the C string array

    for (i = 0; i < len; i++) {
        jstring element = (jstring)(*env)->GetObjectArrayElement(env, jresult, i); // Get the i-th element
        const char* str = (*env)->GetStringUTFChars(env, element, NULL); // Convert jstring to C string
        result[i] = strdup(str); // Duplicate the C string
        (*env)->ReleaseStringUTFChars(env, element, str); // Release the C string
        (*env)->DeleteLocalRef(env, element); // Delete local reference
    }
    result[len] = NULL; // Mark the end of the array with NULL
...
func JoinChordDHT(nodeName string, rootName string, port int) (*ChordDHT, error) {
	var out_error *C.char
	nodeNameC := C.CString(nodeName)        // Convert Go string to C string
	rootNameC := C.CString(rootName)        // Convert Go string to C string
...
func (chord *ChordDHT) DeleteObject() error {

	error := C.delete_global_ref(chord.instance)
	if error != nil {
		return errors.New(C.GoString(error))
	}
	chord.instance = nil // Ensure the instance is set to nil after deletion
	return nil
}
